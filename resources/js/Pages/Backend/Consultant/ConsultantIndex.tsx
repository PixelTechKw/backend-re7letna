...
import { Consultant, PageProps } from "@/types";
...
export default function ({
    elements,
}: PageProps<{ elements: Consultant[] }>): React.ReactNode {
    const {
        ziggy: { location, query },
    } = usePage().props;
    const dispatch = useAppDispatch();

    const columns: ColumnDef<Consultant>[] = useMemo(
        () => [
            {
                accessorKey: "id",
                header: ({ column }: any) => {
                    ...
                    return (
                        ...
                        >
                            <Tooltip>
                                ...
                                >
                                    <p className="text-balance whitespace-pre-line leading-relaxed">
                                        If not active consultants can not
    ...
    return (
        <AuthenticatedLayout header={capitalize("list of consultants")}>
            <div className="w-full flex flex-1 flex-col bg-white  rounded-xl min-h-screen gap-y-4 p-6">
                <div className="flex justify-between items-center">
                    ...
                    <Link
                        href={route("backend.consultant.create")}
